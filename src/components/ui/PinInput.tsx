...
import { useCallback, useId, useRef, useState, type ClipboardEvent, type KeyboardEvent } from 'react';
...
type PinInputProps = {
  ...
  className?: string;
  disabled?: boolean;
  /** Applied to each cell; use `current-password` on sign-in. */
  autoComplete?: string;
};
...
export function PinInput({
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      <label htmlFor={`${id}-0`} className="font-sans text-sm text-ink-secondary dark:text-dark-ink-secondary">
        {label}
      </label>
        ...
        {Array.from({ length: LEN }, (_, i) => {
          ...
          return (
            <input
              ...
              disabled={disabled}
              value={v}
              aria-label={`PIN digit ${i + 1} of ${LEN}`}
              onChange={(e) => handleChange(i, e.target.value)}
              onKeyDown={(e) => handleKeyDown(i, e)}
              onPaste={i === 0 ? handlePaste : undefined}
              className={cn(
                'h-14 min-w-0 flex-1 rounded-[10px] border bg-surface text-center font-sans text-lg font-medium text-ink-primary caret-teal outline-none transition-colors dark:bg-dark-surface dark:text-dark-ink-primary dark:caret-dark-teal',
